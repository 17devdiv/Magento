...
class Queue extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $response += $this->_opportunity->syncAllQueue();
            ...
            $this->messageManager->addSuccess(
                __('All items in queue are synced. Check out Reports for result.')
            );
        } catch (\Exception $e) {
            $response = $this->_serializer->serialize($response);
            $this->messageManager->addError(
                __('Something happen during syncing process. Detail: ' . $e->getMessage() . '. Response Log: ' . $response)
            );
        }
