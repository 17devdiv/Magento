...
class Update extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        if (! empty ( $sellerId )) {
            ...
            if ($action == 'invoice' && $invoiceFlag == 1) {
                $sellerOrderForShippings = $this->_objectManager->get ( 'Dotsquares\Marketplace\Model\Order' )->getCollection ();
    ...
    public function createInvoice($order, $itemsArray, $productSkus, $shippingAmount, $subTotal, $baseSubtotal, $shipVirtualFlag) {
        if ($order->canInvoice ()) {
             ...
             */
            $sellerOrderCollection = $this->_objectManager->get ( 'Dotsquares\Marketplace\Model\Order' )->getCollection ()->addFieldToFilter ( 'order_id', $order->getId () )->addFieldToFilter ( 'seller_id', $customerSession->getId () )->getFirstItem ();
            if (count ( $sellerOrderCollection )) {
                 ...
                 */
                $this->_objectManager->get ( 'Dotsquares\Marketplace\Observer\Invoice' )->updateSellerAmount ( $customerSession->getId (), $totalAmount );
                
                $sellerOrder = $this->_objectManager->get ( 'Dotsquares\Marketplace\Model\Order' )->load ( $sellerOrderCollection->getId () );
