...
class Bulkupload extends \Magento\Framework\App\Action\Action {
    ...
    public function saveProductData($productData, $imagePaths, $folderName) {
        ...
        if (isset ( $productData ['sku'] )) {
            ...
            foreach ( $productData ['sku'] as $key => $value ) {
                ...
                if (! empty ( $productData ['sku'] [$key] ) && in_array ( $productData ['product_type'] [$key], $allowedProductTypes )) {
                    ...
                    $productPrice = $this->_objectManager->create ( 'Dotsquares\Marketplace\Model\Bulkupload' )->setProductDatas ( $productData, $key, $productPrice );
                    $productPrice->save ();
                    $productApproval = $this->_objectManager->create ( 'Dotsquares\Marketplace\Helper\System' )->getProductApproval ();
                    $this->_objectManager->create ( 'Dotsquares\Marketplace\Model\Bulkupload' )->setProductApproval ( $productApproval, $product, $productData, $productId );
        ...
        $fileData = $fInam['name'];
                    ...
                    if (isset ( $fileData ) && $fileData != '') {
                        ...
                        $productBaseImage = $this->_objectManager->create ( 'Dotsquares\Marketplace\Model\Bulkupload' )->setBaseImage ( $productBaseImage, $baseImage, $files, $folderName, $smallImage, $thumbnailImage );
                    ...
                    $this->_objectManager->get ( 'Dotsquares\Marketplace\Controller\Product\Savedata' )->updateStockDataForProduct ( $productId, $productStockData );
                    if (count ( $optionsArray ) >= 1) {
                        $this->_objectManager->get ( 'Dotsquares\Marketplace\Controller\Product\Savedata' )->saveCustomOption ( $productId, $customOptionData );
                    }
            ...
            foreach ( $additionalAttributesArray as $additionalAttributesArr ) {
                ...
                if (in_array ( $additionalAttributeCode, $attributeCodeArray )) {
                    if (! array_key_exists ( $additionalAttributeCode, $attributeLabelValuePair )) {
                        $attributeLabelValuePair = $this->_objectManager->create ( 'Dotsquares\Marketplace\Model\Bulkupload' )->getAttributeOptionValues ( $additionalAttributeCode, $attributeLabelValuePair );
                    }
                    ...
                    if (! empty ( $attr ) && $attr->usesSource ()) {
                        $data [$additionalAttributeCode] = $this->_objectManager->create ( 'Dotsquares\Marketplace\Model\Bulkupload' )->getAdditionalAttributeVales ( $attr, $additionalAttributeValue );
                    } else {
    ...
    public function setAssociatedProductForConfigurable($productId, $productData, $key, $attributeSetId) {
        ...
        if (isset ( $productData ['configurable_variations'] [$key] )) {
            ...
            $simpleProdouctIds = $this->_objectManager->get ( 'Dotsquares\Marketplace\Block\Product\Configurable' )->getAssociatedProductIds ( $simpleProducts );
        }
        ...
        $this->_objectManager->get ( 'Dotsquares\Marketplace\Controller\Product\Savedata' )->associateSimpleProductsWithConfigurable ( $action, $productId, $simpleProductData, $simpleProdouctIds, $attributeSetId, $editSimpleProductFlag );
    }
    /**
    ...
    public function assignDataForDownloadableProduct($productTypeId, $downloadProductId, $downloadableData) {
        if ($productTypeId == 'downloadable' && isset ( $downloadProductId )) {
            ...
            $downloadProductModel = $this->_objectManager->get ( 'Dotsquares\Marketplace\Model\Download' );
		...
		$fInam = $uploader->validateFile();
            ...
            foreach ( $downloadableData as $key => $result ) {
                $downloadProductData = $downloadProductModel->prepareDownloadProductData ( $fInam, $key, $result );
