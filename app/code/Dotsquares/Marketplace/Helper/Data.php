...
class Data extends AbstractHelper {
    ...
    public function showCategoriesTree($categoryName, $catIds) {
        ...
        foreach ( $categoryName as $key => $catname ) {
            ...
            if (strstr ( $key, 'sub' )) {
                ...
                $array = $array . '<li class="level-top  parent" id="' . $key . '"><a href="javascript:void(0);"><span class="end-plus" id="' . $key . '_span"></span></a><span class="last-collapse"><input id="cat' . $key . '" type="checkbox" name="category_ids[]" ' . $catChecked . ' value="' . $key . '"><label for="cat' . $key . '">' . $catname . '<span>(' . $count . ')</span>' . '</label></span>';
            } else {
                $array = $array . '<li class="level-top  parent"><a href="javascript:void(0);"><span class="empty_space"></span></a><input id="cat' . $key . '" type="checkbox" name="category_ids[]"  ' . $catChecked . ' value="' . $key . '"><label for="cat' . $key . '">' . $catname . '<span>(' . $count . ')</span>' . '</label>';
            }
        }
        $array = $array . '</li>';
        $array = $array . '</ul>';
    ...
    public function getSellerRequestpath($productId) {
        ...
        $sellerDatas = $objectManager->get ( 'Dotsquares\Marketplace\Model\Seller' );
