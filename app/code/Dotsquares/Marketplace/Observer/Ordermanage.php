...
class Ordermanage implements ObserverInterface {
     ...
     */
    public function __construct(Data $marketplaceData, \Dotsquares\Marketplace\Helper\System $systemHelper) {
    ...
    public function execute(\Magento\Framework\Event\Observer $observer) {
        ...
        foreach ( $orderItems as $item ) {
            ...
            if (! empty ( $sellerId ) && $item->getParentItemId () == '') {
                $sellerDatas = $objectManager->get ( 'Dotsquares\Marketplace\Model\Seller' )->load ( $sellerId, 'customer_id' );
                ...
                $sellerOrderItemsModel = $objectManager->create( 'Dotsquares\Marketplace\Model\Orderitems' );
                ...
                $isSellerShippingEnabled = $objectManager->get ( 'Magento\Framework\App\Config\ScopeConfigInterface' )->getValue ( 'carriers/dotsquares/active', \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
                $isSellerShippingType = $objectManager->get ( 'Magento\Framework\App\Config\ScopeConfigInterface' )->getValue ( 'carriers/dotsquares/type', \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
                $sellerShippingAmount = 0;
                if ($orderData->getShippingMethod () == 'dotsquares_dotsquares' && $isSellerShippingEnabled == 1 && $item->getIsVirtual () != 1) {
        ...
        foreach ( $sellerData as $sellerIds ) {
            ...
            $products = $objectManager->create ( 'Dotsquares\Marketplace\Model\Orderitems' )->getCollection ();
            ...
            if ($orderData->getShippingMethod () != 'dotsquares_dotsquares') {
            ...
            $sellerOrderModel = $objectManager->create( 'Dotsquares\Marketplace\Model\Order' );
