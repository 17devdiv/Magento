...
namespace Dotsquares\SubscriberRebate\Model\Total\Quote;
    ...
    \Magento\Quote\Model\Quote\Address\Total\AbstractTotal
{
    ...
    /**
     * @var \Dotsquares\SubscriberRebate\Model\Program
     */
    protected $_program;
...
//        die();
        ...
        $amount     = 0;
        if ( $discountOn == \Dotsquares\SubscriberRebate\Model\Config\Source\DiscountOn::SUBTOTAL ) {
            $amount = $total->getBaseSubtotal();
        }
        if ( $discountOn == \Dotsquares\SubscriberRebate\Model\Config\Source\DiscountOn::GRANDTOTAL ) {
    ...
    public function fetch(
        ...
        if ( $discount ) {
            return [
                'code'  => $this->getCode(),
                'title' => __('Subscription Discount'),
                'value' => -$discount,
            ];
        }
    }
